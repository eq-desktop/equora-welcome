...
Singleton {
    ...
    Process {
        ...
        command: ["find", Quickshell.shellDir + "/translations/", "-name", "*.json", "-exec", "basename", "{}", ".json", ";"]
        running: true
    ...
    FileView {
        ...
        path: root.languageCode?.length > 0 ? Qt.resolvedUrl(Quickshell.shellDir + "/translations/" + root.languageCode + ".json") : ""
